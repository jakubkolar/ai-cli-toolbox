...
def _imap_error_handler() -> Iterator[None]:
    """Wrap IMAP operations with standard error handling for all entry points."""
    try:
        yield
...
def _build_reply_body(original: MailMessage, user_body: str, locale: str) -> str:
    ...
    date_str = _format_date_locale(original.date, locale) if original.date.year > 1900 else "Unknown"
...
FILTER BEHAVIOR:
  ...
  Non-ASCII search terms (e.g. accented characters) automatically use
  Gmail's X-GM-RAW extension when connected to a Gmail server.
